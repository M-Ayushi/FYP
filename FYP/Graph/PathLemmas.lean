...
lemma validPath_suffix {n : ℕ} (G : Graph n) (p1 p2 : List (Fin n))
...
lemma isPathFromTo_prefix {n : ℕ} (G : Graph n)
  ...
  isPathFromTo G (p1 ++ [k]) i k := by
    ...
    have hvalid_prefix : validPath G (p1 ++ [k]) := by
      exact validPath_prefix G p1 p2 k hvalid
